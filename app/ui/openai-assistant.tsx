...
export default function OpenAIAssistant({
    ...
    return (
        <div className="flex flex-col bg-black shadow-md relative">
            ...
            <form onSubmit={handleSubmit} className="m-2 flex">
                <input 
                    disabled={isLoading}
                    autoFocus
                    className="border focus:outline-none focus:ring-0 focus:border-white bg-neutral-800 rounded w-full py-2 px-3 text-white" 
                ...
                {isLoading ? 
                    <button 
                        disabled
                        className="ml-2  bg-yellow-500 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">   
                        <OpenAISpinner /> 
                    </button>
                    : 
                    <button 
                        disabled={prompt.length == 0}
                        className="ml-2 bg-yellow-500 hover:bg-yellow-400 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline hover:shadow-yellow-500">   
                        <AiOutlineSend /> 
                    </button>
                }
            </form>
...
export function OpenAIAssistantMessage({message}: {message:Message}) {
    ...
    return (
        <div className="flex rounded text-white text-center bg-zinc-800 px-4 py-2 m-2 shadow-md">
...
function OpenAISpinner() {
    return (
        <svg aria-hidden="true" role="status" className="inline w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-yellow-400" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
